    ...
    <div class="min-h-screen bg-shelf">
        <header class="border-b border-rule bg-paper px-4 py-3">
            <div class="mx-auto flex max-w-2xl items-center justify-between">
                ...
                <a href="/lookup" class="text-sm text-ink-muted hover:text-ink">{{ t('public.search_a_book') }}</a>
            </div>
        </header>
        ...
        <main class="mx-auto max-w-2xl px-4 py-6">
            <div class="rounded-[10px] border border-rule bg-paper p-5">
                ...
                <div class="mt-5">
                    <StatusChip :status="data.copy.status" />
                </div>

                <dl class="mt-5 grid grid-cols-1 gap-3 text-sm sm:grid-cols-2">
                    <div>
                        <dt class="text-ink-subtle">{{ t('public.where_it_is') }}</dt>
                        <dd class="text-ink">{{ data.copy.location || t('public.unknown') }}</dd>
                    </div>
                    <div>
                        <dt class="text-ink-subtle">{{ t('public.can_i_take') }}</dt>
                        <dd class="text-ink">{{ data.for_loan ? t('public.yes') : t('public.not_for_loan') }}</dd>
                    </div>
                    <div>
                        <dt class="text-ink-subtle">{{ t('public.loan_type') }}</dt>
            ...
            <p v-if="data.estimated_available_at && data.available_count === 0" class="mt-3 text-sm text-ink-muted">
                {{ t('public.estimated_around', { date: new Date(data.estimated_available_at).toLocaleDateString() }) }}
            </p>
        </main>
    </div>
</template>
